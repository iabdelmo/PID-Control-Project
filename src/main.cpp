...
int main(int argc, char *argv[])
{
  ...
  pid_angle.Init(0.1, 0.004, 2.0);

//  pid_throttle.Init(0.2, 0.0 , 3.0);
...
//          pid_angle.TWIDDLE = true;
          pid_angle.UpdateError(cte);
          steer_value = pid_angle.TotalError();

          /*Limit the steering angle by the maximum steering angle [-1,1] */
          if (steer_value < -1.0)
          {
              steer_value = -1.0;
          }
